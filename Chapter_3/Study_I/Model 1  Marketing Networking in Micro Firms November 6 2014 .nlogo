...
to setup
  ...
  [
    ...
    set repertoire []
    set prev-product-numb 1 + random ( nrinitproducts - 1 )
...
to setup-nodes
  ...
  [
    ...
    set shape "person"
    set tipo 2
    set choice-num sellerproduct
    set color white
    set sell-capacity  100  ;; random-float
    set infected? true
  ]
end

to setup-spatially-clustered-network
  let num-links (average-node-degree * (nragents + nrsellers)) / 2
...
to go
  sell-products
  make-decision ;; agents make decisions  we need to separate the last procedure
  ask agents with [(tipo = 1) and (infected? = false)] [
  ...
  ask agents with [ tipo = 2 ] [
    set totalproductssold count (link-neighbors with [choice-num = sellerproduct ])
    set totalsellerproduct count (agents with [(tipo = 1) and  (choice-num = sellerproduct)])
  ]
  tick
end

to sell-products
  ask agents with [tipo = 2]  ;; sellers have to do....
  [ask link-neighbors  with [choice-num != sellerproduct]  ;; 
    ;[ if random-float 1 < sell-capacity
      [set choice-num  sellerproduct
       set infected? true]  ;;  I need to change this for the product defined by the user
    ;]
  ]
end
...
507
Potential
(count agents with [(tipo = 1 ) and (choice-num != sellerproduct)]) ;; / (count agents )
3
1
11
...
506
510 degree
count  [(link-neighbors)] of agent 510
17
1
11
...
1318
47
Janssen and Jager (2003) parameters
12
0.0
1
...
"" ""
PENS
"Potential Customer" 1.0 0 -10899396 true "" "plot (count agents with [(tipo = 1 ) and (choice-num != sellerproduct)] )  ; / (count agents )"
"Customers" 1.0 0 -2674135 true "" "plot (count agents with [(tipo = 1 ) and (choice-num = sellerproduct)] )  ; / (count agents )"
